  ...
  <div style="width: 326px; height: 743px;" class="screen">
    ...
    <div class='elements'>
      ...
      <div class='element'>
        ...
        <div class="element-content">           
          <input style=" border-radius: 23px; font-size: 16px;" v-bind:class="{ select: 1 === currentSelect}" oninput="this.value = this.value.replace(/[^0-9.]/g, ''); this.value = this.value.replace(/(\..*)\./g, '$1');" ref="form1" v-model="soldi" class="paragonder" placeholder="$">
          ...
          <button v-bind:class="{ select: 3 === currentSelect}" ref="form3" id="cancella" @click.stop="cancella" class="buton-cancel">{{ IntlString('APP_BANK_BUTTON_CANCEL') }}</button>
        </div>
