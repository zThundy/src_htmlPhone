...
local deleteReceivedMessages = function(identifier)
...
gcPhoneT.deleteAllMessage = function()
    local player = source
    local identifier = gcPhoneT.getPlayerID(player)
    deleteReceivedMessages(identifier)
end
...
AddEventHandler('gcPhone:register_FixePhone', function(phone_number, coords)
	Config.StaticPhones[phone_number] = {name = "TEST"..phone_number, coords = {x = coords.x, y = coords.y, z = coords.z}}
	TriggerClientEvent('gcPhone:register_FixePhone', -1, phone_number, Config.StaticPhones[phone_number])
end)
...
function internal_startCall(player, phone_number, rtcOffer, extraData)
    if Config.StaticPhones[phone_number] ~= nil then
...
function onCallFixePhone(player, phone_number, rtcOffer, extraData)
    ...
    if canCall then
        Chiamate[indexCall] = {
            ...
            rtcOffer = rtcOffer,
            extraData = extraData,
            coords = Config.StaticPhones[phone_number].coords
        }
