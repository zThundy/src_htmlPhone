...
Citizen.CreateThread(function()
    ...
    Citizen.CreateThreadNow(BuildStaticPhones)
end)

function BuildStaticPhones()
    local static_phones = {}

    while true do
        for k, v in pairs(Config.StaticPhones) do
            if GetDistanceBetweenCoords(v.coords.x, v.coords.y, v.coords.z, GetEntityCoords(GetPlayerPed(-1)), true) <= 20.0 then
                ESX.Game.SpawnObject(model, coords, function(obj)
                    FreezeEntityPosition(obj, true)
                    static_phones[k] = obj
                end)
            else
                if DoesEntityExist(static_phones[k]) then
                    DeleteEntity(static_phones[k])
                    static_phones[k] = nil
                end
            end
        end
    ...
    if not inCall then
        ...
        Citizen.CreateThread(function()
            ...
            while inCall do
                ...
                if Config.StaticPhones[infoCall.receiver_num] then
...
AddEventHandler("gcphone:aggiornaRetiWifi", function(retiWifiServer)
    ...
    GLOBAL_WIFI_MODEMS = retiWifiServer

    -- aggiorna la lista del wifi
    SendNUIMessage({event = "updateRetiWifi", data = GLOBAL_WIFI_MODEMS})
...
function startCheck()
    ...
    Citizen.CreateThread(function()
        while true do
            ...
            if isConnected then
                -- print("sei connesso")
                for k, v in pairs(GLOBAL_WIFI_MODEMS) do
...
AddEventHandler('gcPhone:register_FixePhone', function(phone_number, data)
    Config.StaticPhones[phone_number] = data
end)
